...
| **Refactoring technique** | Centralisation de la constante `VISIBLOC_JLG_VERSION`, normalisation booléenne partagée, mécanisme de cache respectant `apply_filters()`, fallback sur échec `plugins_url()`, suppression des dépendances npm obsolètes. | À planifier (voir `REVIEW.md` et `docs/code-review.md`). |
